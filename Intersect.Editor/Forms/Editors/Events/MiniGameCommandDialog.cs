...
internal sealed class MiniGameCommandDialog : Form
{
    ...
    public MiniGameCommandDialog(StartMiniGameCommand command)
    {
        ...
        ClientSize = new Size(620, 680); BackColor = DrawingColor.FromArgb(45, 45, 48); ForeColor = DrawingColor.Gainsboro;
        var layout = new TableLayoutPanel { Dock = DockStyle.Fill, Padding = new Padding(12), ColumnCount = 2, RowCount = 16, AutoScroll = true };
        ...
        Controls.Add(layout);
        var hint = new Label { AutoSize = true, MaximumSize = new Size(560, 0), Margin = new Padding(3, 3, 3, 14),
            Text = "Same map instance + Table ID = shared table. Use identical settings on all access events. " +
                ...
                "TEST chips and progression only: no Aureons are taken or paid. Back up resources/minigames-test.db." };
        ...
        var seats = Number(command.MaxPlayers, 2, 6); var chips = Number(command.StartingChips, 1, 1_000_000_000);
        ...
        AddRow(layout, 3, "Maximum seats (humans + NPCs)", seats); AddRow(layout, 4, "Starting test chips", chips);
        AddRow(layout, 5, "Small blind", small); AddRow(layout, 6, "Big blind", big); AddRow(layout, 7, "Turn timeout (seconds)", seconds);
        AddRow(layout, 8, "Dealer plays and deals", dealer); AddRow(layout, 9, "Other NPC opponents", npcs);
        AddRow(layout, 10, "Automatic hands", automatic); AddRow(layout, 11, "Dealing animation", animation);
        AddRow(layout, 12, "Announce wins in GLOBAL chat", announce); AddRow(layout, 13, "Victory animation (winner only)", victory);
        AddRow(layout, 14, "Dealer / NPC card back", backs);
        var buttons = new FlowLayoutPanel { AutoSize = true, Dock = DockStyle.Fill, FlowDirection = FlowDirection.RightToLeft, Margin = new Padding(3, 14, 3, 3) };
        var cancel = new Button { Text = "Cancel", AutoSize = true, DialogResult = DialogResult.Cancel };
        var save = new Button { Text = "Save", AutoSize = true };
        buttons.Controls.Add(cancel); buttons.Controls.Add(save); layout.Controls.Add(buttons, 0, 15); layout.SetColumnSpan(buttons, 2);
        ...
        save.Click += (_, _) =>
        {
            var draft = new StartMiniGameCommand
            {
                ...
                AnnounceWins = announce.Checked, VictoryAnimationId = ((AnimationChoice)victory.SelectedItem!).Id, NpcCardBackId = backs.SelectedIndex,
            };
            if (!draft.HasValidSettings())
            {
                MessageBox.Show(this, "Use a valid Table ID and blinds. Starting chips must cover the big blind. Reserve one human seat.",
    ...
    {
        layout.Controls.Add(new Label { Text = text, AutoSize = true, Anchor = AnchorStyles.Left, Margin = new Padding(3, 8, 3, 8) }, 0, row);
