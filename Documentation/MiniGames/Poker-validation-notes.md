...
The protocol test program uses the real `IntersectPacket.Data` / `MessagePacker.Deserialize`
engine envelope for request and state packets, not only generic MessagePack DTO serialization.
The in-process two-client model test traverses that envelope in both directions. This still
is not a socket, graphical client, or live gameplay test.
...
The first milestone-3 protocol job completed all 35 core/registry groups and 14 protocol groups
successfully for commit a80ddd78307803d1d9c1700d4215d851251b4efe. Its build log also reported
NU1902/NU1903 vulnerability warnings for the repository's MessagePack 3.1.3 dependency, a
NU1902 warning for NCalcSync 3.8.0, and MessagePack source-generator/analyzer warnings.
...
Consult the PR/checks for the final commit's results; the earlier success does not validate
later changes. Real client/server/editor builds and the manual matrix in Poker.md remain
separate acceptance gates. The branch stays a draft and all stakes remain disposable tests.
