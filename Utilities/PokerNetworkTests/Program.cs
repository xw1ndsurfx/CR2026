...
var tests = new (string Name, Action Test)[]
{
    ...
    {
        var f = new Fixture(); var state = f.Packet(f.A);
        Check(state.IsValid, "Valid snapshot");
        state.State!.Board = [52]; Check(!state.IsValid, "Out of range card accepted");
        state.State.Board = [1, 1]; Check(!state.IsValid, "Duplicate board accepted");
        state.State.Board = []; state.State.Seats = []; Check(!state.IsValid, "Empty live state accepted");
    }),
    ("Recipient snapshots survive serialization without opponent hole cards", () =>
    {
        var f = new Fixture(); f.Start();
        var a = Wire(f.Packet(f.A)); var b = Wire(f.Packet(f.B));
        Check(a.State!.MyCards.Length == 2 && b.State!.MyCards.Length == 2, "Own cards missing");
        Check(!a.State.MyCards.Intersect(b.State!.MyCards).Any(), "Wrong private cards");
        Check(a.State.Seats.All(s => s.RevealedCards.Length == 0), "Opponent cards leaked");
    ...
    ("Per-view guard rejects replay, stale leave and bursts", () =>
    {
        ...
        request.RequestId = 10; request.TableInstanceId = Guid.NewGuid(); Check(!f.A.Guard.Accept(request, 3000), "Wrong table accepted");
    }),
    ("Late acknowledgements clear pending without rolling state back", () =>
    {
        var f = new Fixture(); f.Push();
        var request = f.A.Model.Request(PokerRequestKind.StartHand, 1000)!;
        var ack = f.Packet(f.A, request.RequestId); var newer = f.Packet(f.A);
        newer.State!.Revision += 10;
        ...
        Check(!f.A.Model.Pending && f.A.Model.Current!.State!.Revision == newer.State.Revision, "Late ack rolled state back");
    }),
    ...
    {
        var f = new Fixture(); f.Push();
        var first = f.A.Model.Request(PokerRequestKind.RaiseTo, 1000, 100)!;
        ...
        var refresh = f.A.Model.Request(PokerRequestKind.Refresh, 6000)!;
    ...
    ("Two serialized client models play a complete shared hand", () =>
    {
        ...
        while (f.Snapshot(f.A).Phase != PokerPhase.Finished && ++actions < 32)
        {
            ...
            Check(f.A.Model.Current!.State!.Seats.All(s => s.RevealedCards.Length == 0), "Early reveal");
            f.Process(actor, actor.Model.Current!.State!.ToCall > 0 ? PokerRequestKind.Call : PokerRequestKind.Check);
        }
        ...
        Check(f.A.Model.Current!.State!.Stage == PokerStage.Finished && f.B.Model.Current!.State!.Stage == PokerStage.Finished, "Clients disagree");
        Check(f.A.Model.Current.State.Seats.All(s => s.RevealedCards.Length == 2), "Showdown reveal missing");
    }),
    ...
    ("Late joiners have no private cards until next hand", () =>
    {
        ...
        var state = Wire(PokerTransport.Project(result.Snapshot!));
...
static T Wire<T>(T value) => MessagePackSerializer.Deserialize<T>(MessagePackSerializer.Serialize(value));
...
internal sealed class Fixture
{
    ...
    public void Push(Peer? actor = null, long request = 0)
    {
        foreach (var p in new[] { A, B })
        {
            ...
            var copy = MessagePackSerializer.Deserialize<PokerStatePacket>(MessagePackSerializer.Serialize(packet));
    ...
    public void Process(Peer p, PokerRequestKind kind)
    {
        ...
        request = MessagePackSerializer.Deserialize<PokerRequestPacket>(MessagePackSerializer.Serialize(request));
