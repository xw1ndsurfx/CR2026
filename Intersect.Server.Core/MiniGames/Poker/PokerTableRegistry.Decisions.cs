...
public sealed partial class PokerTableRegistry
{
        ...
        PokerAction action, long raiseTo, DateTimeOffset now)
    {
        ...
        if (actor != null)
        {
            ...
            {
                var allIn = action == PokerAction.RaiseTo && raiseTo >= before.MaximumRaiseTo ||
                    action == PokerAction.Call && before.ToCall >= actor.Chips;
                var kind = allIn ? "allin" : action switch
                    { PokerAction.Fold => "fold", PokerAction.Check => "check", PokerAction.Call => "call", _ => "raise" };
