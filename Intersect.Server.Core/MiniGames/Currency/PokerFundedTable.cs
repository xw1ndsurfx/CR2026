...
/// <summary>
/// Serialized by PokerCurrencyRuntime.Gate. The existing, tested hold'em engine owns cards,
/// betting, side pots and turns. This adapter owns funded seats and durable hand boundaries.
/// It never calls Player, inventory, networking or a client-supplied balance.
/// </summary>
internal sealed class PokerFundedTable
{
    ...
    public PokerFundedTable(PokerMoneyLedger money, PokerTableKey key, Guid currency, PokerRules rules,
        PokerTableOptions options, long reserve)
    {
        _money = money; Key = key; Currency = currency; Rules = rules; Options = options; Reserve = reserve;
        _table = new PokerTable(rules);
        // Deliberately excludes map-instance ID: creating another instance never refills the house.
    ...
    public void Join(MoneySeat escrow, string name)
    {
        ...
        _members.Add(escrow.Character, new Member(escrow, name) { Profile = profile, HandBack = profile.SelectedBack });
        ++Version;
    }
    ...
    public PokerPresentation Presentation(Guid player)
    {
        ...
        return new(_members.Where(p => p.Value.Escrow.Npc).Select(p => p.Key).ToArray(), _dealer, Options.AutoStart, Options.DealAnimationId)
        {
            ...
            Experience = profile.Experience, Wins = profile.Wins, ProgressPending = Pending,
    ...
    public PokerError Start(Guid player, long revision, DateTimeOffset now)
    {
        ...
        Complete(); Cleanup(now); PrepareNpcs(now);
        return StartReady(player, now);
    }
    private PokerError StartReady(Guid player, DateTimeOffset now)
    {
        var result = _table.StartHand(player, now);
        ...
        Decision(dealer.PlayerId, "deal"); ++Version;
    ...
    public PokerError Act(Guid player, long hand, long revision, PokerAction action, long amount, DateTimeOffset now)
    {
        ...
        var result = _table.Act(player, hand, revision, action, amount, now);
    ...
    {
        foreach (var s in Current.Seats.Where(s => s.PlayerId != primary && s.Folded &&
    ...
    public void Leave(Guid player, DateTimeOffset now)
    {
        ...
        m.Leaving = true; ++Version;
        if (Pending) return; // Do not remove a bankroll whose settlement is unresolved.
    ...
    { Pending = true; RetryAt = now.AddSeconds(2); ++Version; }
    ...
    public void Tick(DateTimeOffset now)
    {
        ...
        if (IsEmpty) return;
        // A disconnect received while a previous write failed still has to reach the engine.
        ...
        if (!Playing(state.Phase))
        {
            ...
            var human = state.Seats.FirstOrDefault(s => !_members[s.PlayerId].Escrow.Npc &&
                !_members[s.PlayerId].Leaving && s.Chips > 0);
            if (!Options.AutoStart || human == null || state.Seats.Count(s => !s.Leaving && s.Chips > 0) < 2)
            { _nextHand = null; return; }
            _nextHand ??= now.AddSeconds(5);
            if (now >= _nextHand.Value) { _nextHand = null; StartReady(human.PlayerId, now); }
            return;
        }
        _nextHand = null;
        ...
        var view = Snapshot(next.PlayerId);
        var choice = PokerNpcPolicy.Choose(view, next.PlayerId, Rules.BigBlind);
        ...
        {
            view = Snapshot(next.PlayerId);
            Act(next.PlayerId, view.HandId, view.Revision, view.ToCall == 0 ? PokerAction.Check : PokerAction.Fold, 0, now);
        }
        Cleanup(now);
    }
    private void Complete()
    {
        ...
        var closing = state.Seats.ToDictionary(s => _members[s.PlayerId].Escrow.Id, s => s.Chips);
        _money.Settle(Id, state.HandId, closing); // ALL balances + XP + receipt in one commit.
        // Refresh profiles before publishing/marking this hand handled. A retry is idempotent.
        var profiles = Humans.ToDictionary(id => id, id => _money.Profile(id));
        foreach (var seat in state.Seats)
        {
            var member = _members[seat.PlayerId];
            var net = seat.Chips - member.Escrow.Amount;
    ...
    private void Cleanup(DateTimeOffset now)
    {
        ...
        if (Humans.Length == 0)
            foreach (var p in _members.Keys.ToArray()) Remove(p, now);
    }
    private void Remove(Guid actor, DateTimeOffset now)
    {
        var member = _members[actor];
        _money.Release(member.Escrow.Id); // Durable claim BEFORE removing a seat.
        _table.Leave(actor, now); _members.Remove(actor);
        if (_dealer == actor) _dealer = Guid.Empty;
        ++Version;
    }
    private void PrepareNpcs(DateTimeOffset now)
    {
        ...
        var desired = Math.Min(Options.NpcPlayers, Rules.MaxPlayers - Humans.Length - (Options.DealerPlays ? 1 : 0));
        ...
        if (Options.DealerPlays && _dealer == Guid.Empty && _members.Count < Rules.MaxPlayers)
            AddNpc(PokerTableTheme.DealerName, true);
    ...
    {
        var escrow = _money.OpenNpc(Guid.NewGuid(), Id, Currency, House, Reserve, Rules.StartingChips);
        if (escrow == null) return false; // Empty house: NO refill, no synthetic spendable chips.
    ...
    public FundedWin[] CollectWins()
    { var wins = _wins.ToArray(); _wins.Clear(); return wins; }
}
