...
internal sealed class PokerFundedTable
{
    ...
    private readonly Queue<FundedLevelUp> _levelUps = new();
    ...
    public PokerPresentation Presentation(Guid player)
    {
        ...
        return new(_members.Where(p => p.Value.Escrow.Npc).Select(p => p.Key).ToArray(), _dealer, Options.AutoStart, Options.DealAnimationId)
        {
            ...
            Effects = Options.EffectiveEffects, Decisions = _decisions.ToArray(),
        };
    }
        ...
        if (Current.Revision != before.Revision)
        {
            ...
            if (actor != null)
            {
                if (result == PokerError.None)
                {
                    ...
                    var actionName = afterActor?.AllIn == true && action is PokerAction.Call or PokerAction.RaiseTo
                        ? "allin"
                        : action switch { PokerAction.Fold => "fold", PokerAction.Check => "check", PokerAction.Call => "call", _ => "raise" };
                    Decision(actor.PlayerId, actionName,
    ...
    private void Complete()
    {
        ...
        var oldProfiles = Humans.ToDictionary(id => id, id => _money.Profile(id));
        ...
        foreach (var seat in state.Seats)
        {
            ...
            {
                member.Profile = profiles[seat.PlayerId];
                var oldLevel = MiniGameProgression.Level(oldProfiles[seat.PlayerId].Experience);
                var newLevel = MiniGameProgression.Level(member.Profile.Experience);
                for (var level = oldLevel + 1; level <= newLevel; ++level)
                    _levelUps.Enqueue(new(seat.PlayerId, seat.Name, level));
            }
            if (net <= 0) continue;
    ...
    public FundedLevelUp[] CollectLevelUps() { var values = _levelUps.ToArray(); _levelUps.Clear(); return values; }
