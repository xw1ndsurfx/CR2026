...
public sealed class PokerMoneyLedger : IDisposable
{
    ...
    public MoneySeat? OpenNpc(Guid seat, Guid table, Guid currency, string house, long seed, long stake, bool unlimited = false)
    {
        ...
        return Write<MoneySeat?>((c, tx) =>
        {
            ...
            // Explicit unlimited house mode is a designer-authorized faucet for NPC opponents only.
            // It tops the house up just enough for the next NPC buy-in; player inventory is never credited here.
            if (unlimited && available < stake)
            {
