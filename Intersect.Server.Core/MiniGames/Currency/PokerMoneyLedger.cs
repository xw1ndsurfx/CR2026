...
public sealed record MoneySeat(Guid Id, Guid Table, Guid Character, Guid Currency, string House,
    long Amount, bool Npc, int Status);
...
/// Active balances are checkpoints between hands. On restart an unfinished hand is voided
/// for EVERY participant; a committed result is never undone. No test chips are imported.
/// One game-server process per database: an OS lease prevents concurrent recovery owners.
/// </summary>
public sealed class PokerMoneyLedger : IDisposable
{
    ...
    // Test-only seams: production never assigns these delegates.
    ...
    public PokerMoneyLedger(string connectionString)
    {
        ...
        builder.Mode = SqliteOpenMode.ReadWrite; // Never silently create a replacement player database.
        builder.Pooling = false; builder.DefaultTimeout = 2;
        _connectionString = builder.ToString();
        _lease = new FileStream(builder.DataSource + ".poker.lock", FileMode.OpenOrCreate,
        ...
        try
        {
            ...
            Exec(c, tx, """
                ...
                CREATE TABLE IF NOT EXISTS PokerMoneySeats (
                    ...
                    Npc INTEGER NOT NULL CHECK(Npc IN (0,1)),
                ...
                """);
            // There cannot be another live owner while this process holds the OS lease.
    ...
    private static MoneySeat[] Seats(SqliteConnection c, SqliteTransaction? tx, string where, params object[] values)
    {
        using var command = Command(c, tx,
            "SELECT Id,TableId,CharacterId,Currency,House,Amount,Npc,Status FROM PokerMoneySeats WHERE " + where, values);
        using var reader = command.ExecuteReader(); var result = new List<MoneySeat>();
        while (reader.Read()) result.Add(new(Guid.Parse(reader.GetString(0)), Guid.Parse(reader.GetString(1)),
            Guid.Parse(reader.GetString(2)), Guid.Parse(reader.GetString(3)), reader.GetString(4),
            reader.GetInt64(5), reader.GetInt64(6) != 0, reader.GetInt32(7)));
    ...
    private T Write<T>(Func<SqliteConnection, SqliteTransaction, T> work)
    {
        lock (_gate)
        {
            ...
            var result = work(c, tx); tx.Commit(); return result;
        }
    }

    private static bool HasReceipt(SqliteConnection c, string receipt) =>
        Scalar(c, null, "SELECT 1 FROM PokerMoneyTransfers WHERE Receipt=$p0", receipt) != null;
    ...
    // Only this transaction can call the inventory writer. The caller holds EntityLock and
    // the account autosave lock until success is known AND the live inventory is synchronized.
    private void Transfer(string receipt, Action<SqliteConnection, SqliteTransaction> work)
    {
        lock (_gate)
        {
            using var c = Open();
            if (HasReceipt(c, receipt)) return;
            using var tx = c.BeginTransaction(); var attempted = false;
            try
            {
                work(c, tx);
                Fault?.Invoke("before-commit");
                attempted = true; tx.Commit();
                Fault?.Invoke("after-commit");
            }
            catch (Exception error)
            {
                try { tx.Rollback(); } catch { /* May already be committed. Resolve by receipt. */ }
                if (attempted)
                {
                    try { using var verification = Open(); if (HasReceipt(verification, receipt)) return; }
                    catch (Exception verificationError)
                    {
                        // Continuing with a stale live inventory would allow duplication on autosave.
                        var fatal = new AggregateException("Cannot resolve a poker inventory commit; restart is required.", error, verificationError);
                        if (UnverifiableCommit != null) UnverifiableCommit(fatal);
                        else Environment.FailFast(fatal.Message, fatal);
                        throw fatal;
                    }
                }
                throw;
    ...
    public MoneySeat OpenHuman(Guid seat, Guid table, Guid character, Guid currency, string house, long amount,
        Action<SqliteConnection, SqliteTransaction> debit)
    {
        Required(seat); Required(table); Required(character); Required(currency); Amount(amount);
        if (amount == 0) throw new MoneyRuleException("A buy-in must be positive.");
        var expected = new MoneySeat(seat, table, character, currency, house, amount, false, 0);
        Transfer(Id(seat) + ":in", (c, tx) =>
        {
            InsertSeat(c, tx, expected);
            debit(c, tx);
            Receipt(c, tx, Id(seat) + ":in", character, currency, amount, "buy-in");
        });
        return expected;
    }
    ...
    public MoneySeat? OpenNpc(Guid seat, Guid table, Guid currency, string house, long seed, long stake)
    {
        ...
        return Write<MoneySeat?>((c, tx) =>
        {
            ...
            var result = new MoneySeat(seat, table, Guid.Empty, currency, house, stake, true, 0);
            InsertSeat(c, tx, result); return result;
        });
    }
    ...
    private static void ReturnNpc(SqliteConnection c, SqliteTransaction tx, MoneySeat s)
    {
        ...
        Exec(c, tx, "UPDATE PokerMoneyHouses SET Available=Available+$p1 WHERE House=$p0", s.House, s.Amount);
    ...
    public void Release(Guid seat) => Write((c, tx) =>
    {
        ...
        if (s.Npc) ReturnNpc(c, tx, s);
        else Exec(c, tx, "UPDATE PokerMoneySeats SET Status=1 WHERE Id=$p0 AND Status=0", Id(seat));
        return true;
    });
    public MoneySeat[] Refunds(Guid character)
    {
        Required(character);
        lock (_gate) { using var c = Open(); return Seats(c, null, "CharacterId=$p0 AND Npc=0 AND Status=1", Id(character)); }
    }
    public void Cashout(MoneySeat expected, Action<SqliteConnection, SqliteTransaction> credit)
    {
        ...
        Transfer(Id(expected.Id) + ":out", (c, tx) =>
        {
            var s = Seats(c, tx, "Id=$p0", Id(expected.Id)).SingleOrDefault();
            if (s == null || s.Npc || s.Status != 1 || s.Amount != expected.Amount ||
                s.Character != expected.Character || s.Currency != expected.Currency)
    ...
    public MoneyWin[] Settle(Guid table, long hand, IReadOnlyDictionary<Guid, long> closing)
    {
        ...
        return Write((c, tx) =>
        {
            ...
            if (old != null)
            {
                if (old != fingerprint) throw new MoneyRuleException("A settled hand cannot change its result.");
                return Array.Empty<MoneyWin>();
            }
            var seats = Seats(c, tx, "TableId=$p0 AND Status=0", Id(table));
            if (seats.Length != closing.Count || seats.Any(s => !closing.ContainsKey(s.Id)) ||
            ...
            {
                var amount = closing[s.Id];
                Exec(c, tx, "UPDATE PokerMoneySeats SET Amount=$p1 WHERE Id=$p0", Id(s.Id), amount);
                if (!s.Npc && amount > s.Amount)
                {
                    var profile = ReadProfile(c, tx, s.Character).WithWin();
                    WriteProfile(c, tx, s.Character, profile);
                    wins.Add(new(s.Character, amount - s.Amount));
                }
            }
            ...
            Fault?.Invoke("before-settlement-commit");
            return wins.ToArray();
        });
    }
    private static MiniGameProgress ReadProfile(SqliteConnection c, SqliteTransaction? tx, Guid character)
    {
        using var command = Command(c, tx, "SELECT Experience,Wins,SelectedBack FROM PokerMoneyProfiles WHERE CharacterId=$p0 AND Game='poker'", Id(character));
        using var reader = command.ExecuteReader();
        var result = reader.Read() ? new MiniGameProgress(reader.GetInt64(0), reader.GetInt64(1), reader.GetInt32(2)) : new();
        if (!result.IsValid) throw new MoneyRuleException("Invalid funded progression; refusing to reset it.");
        return result;
    }
